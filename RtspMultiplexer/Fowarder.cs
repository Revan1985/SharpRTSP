...
public abstract class Forwarder
{
    ...
    /// <summary>
    /// The number of RTP index watched to detect missing packet
    /// </summary>
    private const int WATCHED_FRAME_INDEX = 10;
    ...
    /// </summary>
    protected UdpClient ForwardVUdpPort { get; }
    /// <summary>
    /// Listen UDP port (receive data on this port)
    /// </summary>
    protected UdpClient ListenCUdpPort { get; }
    ...
    private void InitReceivedFrame(ushort aFirstIndex)
    {
        ...
        for (int i = (ushort)(aFirstIndex - WATCHED_FRAME_INDEX); i != aFirstIndex; i++)
        {
    ...
    protected void VideoFrameSended(int nbOfByteSend, byte[] frame)
    {
        lock (ForwardVUdpPort)
        {
            ...
            if (_logger.IsWarnEnabled)
            {
                ...
                ushort oldIndex = (ushort)(newRTSPFrameIndex - WATCHED_FRAME_INDEX);
    ...
    {
        OnCommandReceive(EventArgs.Empty);
